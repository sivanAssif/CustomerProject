...
test('full onboarding flow with stage gating', async ({ page }) => {
  ...
  // הגענו לעמוד הלקוח, כל 3 השלבים גלויים
  await expect(page.getByRole('heading', { name, level: 1 })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'הכנה והגדרת הפרויקט' })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'יישור קו עם הצוותים' })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'פיתוח, בדיקות ותיעדוף' })).toBeVisible();
  ...
  // שלבים 2 ו-3 נעולים למילוי אך גלויים לצפייה
  await expect(
    page.getByText('🔒 גלוי לצפייה — נפתח בשלב הקודם').first()
  ).toBeVisible();
  await expect(page.getByText('🔒 גלוי לצפייה — נפתח בשלב הקודם')).toHaveCount(2);
});
